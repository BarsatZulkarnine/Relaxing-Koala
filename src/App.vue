    ...
    <nav class="ml-auto flex gap-4 sm:gap-6">
      ...
      <div v-if="isSignedIn" class="relative">
        <button @click.stop="toggleDropdown"
          class="text-base font-medium hover:underline underline-offset-4 focus:outline-none">
        ...
        <transition name="fade">
          <div v-if="showDropdown"
            ...
            <button v-for="option in accountOptions" :key="option.name" @click="option.action"
              class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ option.name }}</button>
          </div>
        </transition>
      </div>
    </nav>
